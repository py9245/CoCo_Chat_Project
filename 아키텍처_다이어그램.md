...
## 2. 주요 구성 요소
- **사용자 브라우저**: GitHub Pages 도메인(`https://<username>.github.io`) 또는 EC2 도메인(`https://3-37-130-181.nip.io`)로 접속합니다. 프런트 코드에서 `window.API_BASE_URL` 또는 `VITE_API_BASE_URL`을 참조해 API 엔드포인트를 계산합니다.
...
2. **REST API**  
   - 브라우저 → `https://3-37-130-181.nip.io/api/*`  
